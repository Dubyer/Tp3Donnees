...
export default class Repository {
    ...
    getAll(HttpContext) {
        ...
        // We check if there is a filter in the request (if there is no filter, we don't need to loop on the objectsList)
        ...
        // If there is a filter, we loop on the objectsList to find the data that match the filter and we add them to the bindedDatas array
        if (objectsList) {
            for (let data of objectsList) {
                if (containsFilter) {
                    for (let i = 0; i < Object.keys(HttpContext.path.params).length; i++) {
                        if (Object.keys(HttpContext.path.params)[i] != "limit" && Object.keys(HttpContext.path.params)[i] != "offset" &&
                            Object.keys(HttpContext.path.params)[i] != "fields" && Object.keys(HttpContext.path.params)[i] != "sort") {
                            // If the value is a string, we compare it in uppercase (to avoid case sensitive problems)
                            if (HttpContext.path.params[Object.keys(HttpContext.path.params)[i]].includes('*')) {
                                if (HttpContext.path.params[Object.keys(HttpContext.path.params)[i]].startsWith("*") && HttpContext.path.params[Object.keys(HttpContext.path.params)[i]].endsWith("*")) {
                                    ...
                                    // If the value is a string, we compare it in uppercase
                                    if (typeof (value) == String && type0f(data[Object.keys(HttpContext.path.params)[i]]) == String) {
                                        ...
                                        if (data[Object.keys(HttpContext.path.params)[i]].includes(value)) {
                                            bindedDatas.push(this.model.bindExtraData(data));
                                            // We add the data to the bindedDatas array
                                        }
                                    }
                                }
        ...
        // Here we remove duplicates from the array
        const seen = new Set();
